...
appControllers.controller('storeCtrl', ['$scope', 'ParseFactory', '$routeParams', 'storeLocalStorage', '$rootScope',
    function($scope, ParseFactory, $routeParams, storeLocalStorage, $routeScope) {

        ParseFactory.init();

        //Set the Page Title + the meta tags
        $routeScope.pageTitle = $routeParams.storeHandle;
        ...
        if ($scope.storeData == null) {
            ...
            ParseFactory.getStoreByHandle($scope.storeHandle).then(
                function(store) {
                    ...
                    //Set the title and Meta Data Once we get the Data
                    $routeScope.pageTitle = $scope.storeData.get('name');
                    $routeScope.metaKeyWords = "Sample, Keywords";
                    $routeScope.metaDescription = "Sample Description";
        ...
        $scope.addReviewToStore = function() {
            ...
            if ($scope.newReview.trim() != "" && $scope.hasReviewed == false) {
...
appControllers.controller('dashboardCtrl', ['$scope', 'ParseFactory', '$routeParams', '$rootScope',
    function($scope, ParseFactory, $routeParams, $routeScope) {
        //Set the Page Title + the meta tags
        $routeScope.pageTitle = "DashBoard";
        $routeScope.metaKeyWords = "Sample, Keywords";
        $routeScope.metaDescription = "Sample Description";

        $scope.storeHandle = "store";
    }
]);